...
import { Plus, CheckCircle2, FileText, X, User, Mail, Phone, Briefcase, MapPin } from 'lucide-react';
import { onboardingCandidates, contacts } from '@/data/mock-data';
import type { OnboardingCandidate, CandidateStatus, CandidatePath, Contact } from '@/types/crm';
...
export default function CandidatesPage() {
  ...
  const openNewForm = () => {
    ...
    setFormData({
      ...
      path: 'B2B seeking Contracts', cvFileName: '', hourlyRateEur: 0,
      b2bEntityName: '', appliedDate: new Date().toISOString().split('T')[0],
      selectedSlots: [], confirmedSlotId: '', reviewerNotes: '',
      reviewedBy: '', status: 'Applied', createdContactId: '', createdAccountId: '',
    });
  };
  ...
  }, [statusFilter, pathFilter, searchTerm, appliedDateFilter, colFilters, candidates]);
  ...
  return (
    <div>
      ...
      <div className="space-y-3 mb-4">
        ...
        {(searchTerm || (statusFilter && statusFilter !== 'All') || pathFilter || appliedDateFilter.type !== 'all') && (
          <div className="flex items-center gap-2 flex-wrap text-xs">
            ...
            <Button variant="outline" size="sm" className="h-7 text-xs"
              onClick={() => { setSearchTerm(''); setStatusFilter('All'); setPathFilter(''); setAppliedDateFilter({ type: 'all' }); }}>
              Clear all
            </Button>
          </div>
        )}
      ...
      <div className="rounded-lg border">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No candidates match the current filters.</TableCell></TableRow>
                      ...
                      <>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                            CV File <span className="text-destructive text-sm leading-none">*</span>
                          </label>
                          <button
                            onClick={() => setViewCvFileName(formData.cvFileName)}
      ...
      {/* CV Preview Dialog */}
      <Dialog open={!!viewCvFileName} onOpenChange={() => setViewCvFileName(null)}>
        <DialogContent className="max-w-3xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <FileText className="h-5 w-5" /> {viewCvFileName}
            </DialogTitle>
          </DialogHeader>
          <div className="bg-muted/30 rounded-lg p-8 min-h-[400px] flex flex-col items-center justify-center border">
            <FileText className="h-16 w-16 text-muted-foreground/40 mb-4" />
            <p className="text-sm text-muted-foreground mb-1">PDF Preview</p>
            <p className="text-xs text-muted-foreground">In production, the actual CV PDF would be rendered here.</p>
            <p className="text-xs font-medium mt-4">{viewCvFileName}</p>
          </div>
        </DialogContent>
      </Dialog>
