...
import type { Invoice, InvoiceLine, BusinessEntity } from '@/types/crm';
import { getEntityById, getAccountById, getContactById } from '@/data/mock-data';
import { formatDate } from '@/lib/format';
import cspLogoUrl from '@/assets/csp-logo.png';
...
/** Build the payment details lines based on entity country */
function getPaymentLines(entity: BusinessEntity, invoiceCurrency?: string): Array<{ heading?: string; lines: [string, string][] }> {
  const sections: Array<{ heading?: string; lines: [string, string][] }> = [];

  if (entity.country === 'Bulgaria') {
    // For GBP invoices use UK bank, otherwise EU bank
...
export async function generateInvoicePdf(invoice: Invoice): Promise<Blob> {
  const entity = getEntityById(invoice.entityId);
  const account = getAccountById(invoice.accountId);
  if (!entity || !account) throw new Error('Missing entity or account');
  ...
  // Load logo
  try {
    const logoData = await loadImageAsDataUrl(cspLogoUrl);
    doc.addImage(logoData, 'PNG', 14, 10, 30, 22);
  } catch { /* continue without logo */ }

  // Entity details (top right) — use country settings, wrap address to fit
  doc.setFontSize(8);
  doc.setTextColor(80, 80, 80);
  const ENTITY_BLOCK_WIDTH = 75; // mm
  ...
  // Company name + Invoice title — push down if entity block is tall
  let y = Math.max(42, entityY + 4);
  doc.setFontSize(22);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(HEADER_COLOR[0], HEADER_COLOR[1], HEADER_COLOR[2]);
  // Wrap company name within the available width to the left of "Invoice"
  ...
  // Account name (bold) + wrapped address below
  doc.setTextColor(0, 0, 0);
  doc.setFont('helvetica', 'bold');
  doc.text(account.name, 42, y);

  // Invoice metadata (right side) — pinned to same starting Y as Bill To
  let metaY = y;
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(100, 100, 100);
  ...
  const tableBody = lines.map((line: InvoiceLine) => {
    const contact = line.contactId ? getContactById(line.contactId) : null;
    const desc = contact
      ? `${contact.firstName} ${contact.lastName} - ${line.description}`
      : line.description;
    return [
      desc,
  ...
  // RON Exchange — render right under the table when present
  ...
  // Determine invoice currency from first line
